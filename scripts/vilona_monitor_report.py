...
def main():
    ...
    report = {"strategies": {}, "accounts": {}, "heartbeats": {}, "anomalies": [], "summary": {}}
    ...
    with engine.connect() as conn:
        ...
        for r in rows:
            pf = abs(r[1]/r[2]) if r[2] != 0 else 999
        ...
        rows = conn.execute(text("""
            ...
            FROM trades WHERE timestamp >= NOW() - INTERVAL '24 hours'
        """)).fetchall()
        ...
        # Exception: profitable low-WR strategies (big wins cover losses) are monitored
        for name, strat in report["strategies"].items():
            if strat["trades_7d"] >= 10 and strat["win_rate"] < 0.50:
                if strat["pnl_7d"] < 0:
                    # Losing + low WR = CRITICAL — must be disabled/improved
                    report["anomalies"].append({
                    ...
                    # Profitable + low WR = WARNING — monitor closely
                    report["anomalies"].append({
                        "type": "low_wr_warning",
                        "strategy": name,
