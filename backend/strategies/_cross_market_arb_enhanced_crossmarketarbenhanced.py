...
from ._cross_market_arb_enhanced_arbopportunityenhanced import (
...
class CrossMarketArbEnhanced:
    ...
    def scan_all_providers(
        ...
        """
        start = _facade.time.monotonic()
        all_opps: _facade.List[_facade.ArbOpportunityEnhanced] = []
        total_markets = 0
        ...
        elapsed = (_facade.time.monotonic() - start) * 1000
        return _facade.ScanResult(
    ...
    def detect_yes_no_sum(
        ...
        fee = market.get("fee_pct", _facade._DEFAULT_FEES.get(platform, 0.02))
        ...
        return _facade.ArbOpportunityEnhanced(
            event_id=market_id,
            kind="yes_no_sum",
            platform_a=platform,
            platform_b=platform,
    ...
    def detect_complementary(
        ...
        opportunities = []
        event_groups: _facade.Dict[str, _facade.List[_facade.Dict[str, _facade.Any]]] = {}
        ...
        for eid, group in event_groups.items():
            ...
            for m in group:
                ...
                try:
                    ...
                    _facade.logger.debug("cross_market_arb: failed to parse yes_price")
            ...
            fee_per = group[0].get("fee_pct", _facade._DEFAULT_FEES.get(platform, 0.02))
            ...
            if net > 0:
                ...
                opp = _facade.ArbOpportunityEnhanced(
                    event_id=eid,
                    kind="multi_outcome",
                    platform_a=platform,
                    platform_b=platform,
        ...
        for ma in markets_a:
            ...
            for mb in markets_b:
                ...
                if ea and eb and ea == eb:
                    ...
                    matched = True
                elif _facade._questions_match(qa, qb):
                    matched = True
                if not matched:
                    continue
    ...
    def _compute_generic_arb(
        ...
        fee_a = m_a.get("fee_pct", _facade._DEFAULT_FEES.get(platform_a, 0.02))
        fee_b = m_b.get("fee_pct", _facade._DEFAULT_FEES.get(platform_b, 0.02))
    ...
    def detect_same_game_correlation(
        ...
        game_groups: _facade.Dict[str, _facade.List[_facade.Dict]] = {}
        for m in markets:
            q = m.get("question", "")
            if " vs " in q:
                parts = q.split(" vs ")
        ...
        for game_key, group in game_groups.items():
            ...
            ml_groups: _facade.Dict[str, _facade.List[_facade.Dict]] = {}
            for m in moneylines:
                raw = m.get("_raw", {})
                        ...
                        for mb in ml_groups[teams[j]]:
                            ...
                            if yes_sum < 0.96:  # Leave room for 2% fee per side
                                ...
                                if net <= self.min_net_profit_pct:
                                    continue
                                opps.append(
                                    _facade.ArbOpportunityEnhanced(
                                        event_id=game_key,
                                        kind="same_game_ml",
    ...
    def detect_cross_timeframe_arb(
        ...
        asset_groups: _facade.Dict[str, _facade.List[_facade.Dict]] = {}
        for m in markets:
            q = m.get("question", "")
            match = _re.search(
