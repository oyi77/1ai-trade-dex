    ...
    while cs < now:
        async with httpx.AsyncClient(timeout=30) as c:
            r = await c.get(
                f"{BINANCE}/klines",
                params=dict(
                    symbol="BNBUSDT", interval=interval, limit=1000, startTime=cs
                ),
            )
            r.raise_for_status()
            k = r.json()
...
def bb_s(c: List[float], p=20, s=2.0):
    ...
    for i in range(len(c)):
        if i < p - 1:
            ...
            w = c[i + 1 - p : i + 1]
            sd = math.sqrt(sum((x - mid[i]) ** 2 for x in w) / p)
            up.append(mid[i] + s * sd)
            lo.append(mid[i] - s * sd)
    return up, mid, lo
...
def bt(k: List[List], s: Strategy, p: dict) -> Result:
    ...
    if in_pos:
        ...
        pnl = ev - e_usdc
        trades.append(
            Trade(et or t[-1], t[-1], ep, c[-1], (c[-1] - ep) / ep * 100, pnl, "end")
        )
        cap = cap + ev

    win = [x for x in trades if x.pnl_usdc > 0]
    lose = [x for x in trades if x.pnl_usdc <= 0]
    ret = cap - INIT_CAP
    ret_p = (ret / INIT_CAP) * 100
    ...
    gw = sum(x.pnl_usdc for x in win)
    gl = abs(sum(x.pnl_usdc for x in lose))
    pf = gw / gl if gl > 0 else float("inf")

    dr = []
    for d in range(1, len(eq)):
        if eq[d - 1] != 0:
            dr.append((eq[d] - eq[d - 1]) / eq[d - 1])
    sh = 0
    if len(dr) > 1:
        ad = sum(dr) / len(dr)
    ...
    hold = (
        sum(
            (x.exit - x.entry).total_seconds() / 60
            for x in trades
            if x.exit and x.entry
        )
        / len(trades)
        if trades
        else 0
...
def print_r(r: Result, tag: str = ""):
    ...
    desc = f"{r.name.upper()}"
    if "sf" in p:
        desc += f" SMA({p['sf']}/{p['ss']})"
    if "rsi_p" in p:
        desc += f" RSI({p['rsi_p']}) {p.get('entry_mode', '')}"
    if "bb_p" in p:
        desc += f" BB({p['bb_p']}/{p['bb_s']})"
    desc += f" SL:{p.get('sl', 0) * 100:.0f}% TP:{p.get('tp', 0) * 100:.0f}% Pos:{p.get('pos', 0) * 100:.0f}%"
    print(f"  [{tag}] {desc}")
    print(
        f"    Ret: {r.ret_pct:>+8.2f}%  Win: {r.wr:>5.1f}%  N:{r.n:>3d}  "
        f"DD:{r.dd:>5.2f}%  Sharpe:{r.sharpe:>6.2f}  PF:{r.pf:>6.2f}"
    )
    print(
        f"    W:{r.w} L:{r.l}  AvgW:{r.avg_w:+.2f}%  AvgL:{r.avg_l:+.2f}%  "
...
def save(rs: List[Result], name: str):
    path = Path(
        f"data/backtests/bnb_hack_{name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
async def main():
    ...
    for strat in strats:
        ...
        print("\n  WORST 5:")
        for i, r in enumerate(results[-5:]):
            print_r(r, str(len(results) - 4 + i))

        # Best Sharpe (min 3 trades)
        sh = sorted(
            [r for r in results if r.n >= 3], key=lambda x: x.sharpe, reverse=True
        )
        ...
        if len(k) > 500:
            ...
            for i, r in enumerate(recent_r[:3]):
                print_r(r, f"R{i + 1}")
