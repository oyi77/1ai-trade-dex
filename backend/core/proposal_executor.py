...
class ProposalExecutor:
        ...
        try:
            ...
            with get_db_session() as db:
                ...
                old_config_snapshot = {
                    ...
                    "params": (
                        json.loads(current_config.params)
                        if current_config.params
                        else {}
                    ),
                }
                ...
                new_params = (
                    json.loads(current_config.params) if current_config.params else {}
                )
            ...
            db.rollback()
            self.logger.error(
                f"Failed to execute proposal {proposal_id}: {e}", exc_info=True
            )
            return False
        ...
        try:
            ...
            with get_db_session() as db:
                ...
                if not proposal.executed_at:
                    self.logger.error(
                        f"Proposal {proposal_id} missing executed_at timestamp"
                    )
                    return None
                ...
                # Define time windows
                window_start = execution_time - timedelta(
                    hours=self.IMPACT_WINDOW_HOURS
                )
            ...
            self.logger.error(
                f"Failed to measure impact for proposal {proposal_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    def auto_rollback_if_negative(self, proposal_id: int) -> bool:
        ...
        try:
            ...
            with get_db_session() as db:
                ...
                if impact is None:
                    self.logger.debug(
                        f"No impact measurement available for proposal {proposal_id}"
                    )
                    return False
                ...
                current_config.params = json.dumps(
                    old_config_snapshot.get("params", {})
                )
            ...
            db.rollback()
            self.logger.error(
                f"Failed to rollback proposal {proposal_id}: {e}", exc_info=True
            )
            return False
    ...
    def _get_executed(self, db, limit: int) -> List[Dict[str, Any]]:
        proposals = (
            db.query(StrategyProposal)
            .filter(StrategyProposal.admin_decision == "executed")
            .order_by(StrategyProposal.executed_at.desc())
            .limit(limit)
            .all()
        )
...
async def measure_impact_and_rollback_job():
    ...
    try:
        ...
        logger.info(
            f"Measuring impact for {len(executed_proposals)} executed proposals"
        )
        ...
        if rollback_count > 0:
            logger.warning(
                f"Auto-rolled back {rollback_count} proposals due to negative impact"
            )
        else:
