...
def validate_trade(trade: TradeConfig, portfolio: PortfolioState) -> Tuple[bool, str]:
    ...
    if trade.size_usd > portfolio.free_capital:
        return (
            False,
            f"Insufficient capital: need ${trade.size_usd:.2f}, have ${portfolio.free_capital:.2f}",
        )
    ...
    if portfolio.positions_in_market >= portfolio.max_per_market:
        return (
            False,
            f"Max positions in market reached: {portfolio.positions_in_market}/{portfolio.max_per_market}",
        )
    ...
    if portfolio.category_exposure_pct >= portfolio.max_category_pct:
        return (
            False,
            f"Category exposure too high: {portfolio.category_exposure_pct:.1%} >= {portfolio.max_category_pct:.0%}",
        )
    ...
    if abs(portfolio.daily_loss_usd) >= portfolio.max_daily_loss_usd:
        return (
            False,
            f"Daily loss limit reached: ${abs(portfolio.daily_loss_usd):.2f} >= ${portfolio.max_daily_loss_usd:.2f}",
        )
    ...
    if trade.size_usd < portfolio.min_position_usd:
        return (
            False,
            f"Position too small: ${trade.size_usd:.2f} < ${portfolio.min_position_usd:.2f}",
        )

    if trade.size_usd > portfolio.max_position_usd:
        return (
            False,
            f"Position too large: ${trade.size_usd:.2f} > ${portfolio.max_position_usd:.2f}",
        )
