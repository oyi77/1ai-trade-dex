...
class LearningSystem:
    ...
    def record_outcome(
        ...
        if self.mode == LearningMode.ONLINE:
            ...
            self._log.info(f"Offline learning - example not saved to DB: {strategy_key}")
    ...
    def compute_calibration(self, domain: str) -> CalibrationReport:
        ...
        brier_score = np.mean([(e.prediction - (1.0 if e.actual >= 0.5 else 0.0))**2 for e in examples])
        
        # Calibration bins (10 bins from 0.0 to 1.0)
        num_bins = 10
        bin_data = [[0.0, 0.0, 0] for _ in range(num_bins)]  # [sum_conf, sum_acc, count]
        
        for e in examples:
            bin_idx = min(int(e.confidence * num_bins), num_bins - 1)
            bin_data[bin_idx][0] += e.confidence
            bin_data[bin_idx][1] += 1 if (e.prediction >= 0.5) == (e.actual >= 0.5) else 0
    ...
    def get_learning_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            domain_counts = (
                self.session.query(
                    LearningExampleModel.domain,
                    func.count(LearningExampleModel.domain)
                )
            ...
            for domain, count in domain_counts:
                ...
                if examples:
                    ...
                    stats["domains"][domain]["latest_example"] = examples[0].timestamp.isoformat()
