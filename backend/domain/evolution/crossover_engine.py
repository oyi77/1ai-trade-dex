...
def crossover_genomes(
    ...
    if parent_a.fitness_metrics.sharpe_ratio <= 0.5:
        raise ValueError(f"Parent A not ELITE (sharpe={parent_a.fitness_metrics.sharpe_ratio})")
    if parent_b.fitness_metrics.sharpe_ratio <= 0.5:
        raise ValueError(f"Parent B not ELITE (sharpe={parent_b.fitness_metrics.sharpe_ratio})")
    ...
    for chromosome_name in ["perception", "cognition", "execution", "risk", "meta"]:
        if market_regime == "volatile":
            ...
            if chromosome_name == "risk":
                winner = parent_a if parent_a.fitness_metrics.max_drawdown_pct < parent_b.fitness_metrics.max_drawdown_pct else parent_b
            else:
            ...
            if chromosome_name == "cognition":
                winner = parent_a if parent_a.fitness_metrics.alpha_per_trade > parent_b.fitness_metrics.alpha_per_trade else parent_b
            else:
                winner = parent_a if random.random() < 0.5 else parent_b
            ...
            if chromosome_name == "execution":
                winner = parent_a if parent_a.fitness_metrics.win_rate > parent_b.fitness_metrics.win_rate else parent_b
            else:
                winner = parent_a if random.random() < 0.5 else parent_b
        ...
        child.chromosomes[chromosome_name] = deepcopy(winner.chromosomes[chromosome_name])
