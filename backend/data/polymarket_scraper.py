...
POLYMARKET_BUILDER_LEADERBOARD = (
    f"{POLYMARKET_DATA}/{settings.DATA_API_VERSION}/builder-leaderboard"
)
...
class PolymarketScraper:
    ...
    def _parse_leaderboard_entry(
        ...
        try:
            ...
            # Get volume/amount
            volume = float(
                entry.get("volume") or entry.get("vol") or entry.get("amount") or 0.0
            )
            ...
            score = max(
                ...
                min(
                    1.0,
                    max(0.0, pnl / max(1, abs(pnl))) * 0.7 + (1.0 / max(1, rank)) * 0.3,
                ),
            )

            # Market diversity
            market_diversity = (
                min(1.0, unique_markets / 50) if unique_markets > 0 else 0.0
            )
    ...
    async def fetch_builder_leaderboard(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        now = time.time()
        if (
            _BUILDER_LEADERBOARD_CACHE
            and (now - _BUILDER_LEADERBOARD_TS) < _BUILDER_CACHE_TTL
        ):
            logger.debug(
                "Returning cached builder leaderboard (%d entries)",
                len(_BUILDER_LEADERBOARD_CACHE),
            )
        ...
        try:
            ...
            logger.info(
                "Successfully fetched %d builders from builder leaderboard",
                len(builders),
            )
            return builders
    ...
    def _parse_builder_entry(self, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            profit_pct = float(
                entry.get("profitPercent") or entry.get("profit_percent") or 0.0
            )
    ...
    async def fetch_active_markets(
        ...
        """
        logger.info(
            f"Fetching real markets from Gamma API (category: {category or 'all'}, limit: {limit})"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully fetched {len(markets)} real markets from Gamma API"
            )
            return markets
    ...
    def _parse_market_entry(self, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            liquidity = float(
                entry.get("liquidity") or entry.get("liquidityMeasure") or 0.0
            )
            ...
            category = (
                ...
                else entry.get("category", "General")
            )
