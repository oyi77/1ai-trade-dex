...
_kraken_breaker = CircuitBreaker("kraken", failure_threshold=settings.CB_FAILURE_THRESHOLD, recovery_timeout=settings.CB_RECOVERY_TIMEOUT)
    ...
    async def get_btc_price(self) -> float:
        async def _fetch():
            async with httpx.AsyncClient(timeout=10.0) as client:
                resp = await client.get(f"{self.manifest().base_url}/Ticker", params={"pair": "XXBTZUSD"})
    ...
    async def get_klines(self, symbol: str, interval: str, limit: int) -> list:
        ...
        async def _fetch():
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                pair_map = {"XBTUSD": "XXBTZUSD", "ETHUSD": "XETHZUSD", "SOLUSD": "XSOLZUSD"}
