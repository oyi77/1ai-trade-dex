...
async def test_btc_oracle_does_not_place_direct_clob_orders():
    class Market:
        ...
        slug = "btc-5m-test"
        window_end = __import__("datetime").datetime.now(
    ...
    with (
        patch(
            "backend.strategies.btc_oracle.fetch_btc_price",
            AsyncMock(return_value=100_000.0),
        ),
        patch(
            "backend.data.crypto.compute_btc_microstructure",
