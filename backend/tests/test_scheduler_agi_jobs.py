...
class TestAGIJobConfigDefaults:
    def test_self_review_settings_exist(self):
        from backend.config import ConfigRegistry
...
class TestEvolutionSchedulerRegistration:
    def test_scheduler_uses_configured_evolution_intervals(self):
        ...
        scheduler_file = os.path.join(
            workspace_root, "backend", "core", "scheduling", "scheduler", "_scheduler_setup_c.py"
        )
        if not os.path.exists(scheduler_file):
            scheduler_file = os.path.abspath(
                os.path.join(
                    os.path.dirname(__file__),
                    "../core/scheduling/scheduler/_scheduler_setup_c.py",
                )
            )
