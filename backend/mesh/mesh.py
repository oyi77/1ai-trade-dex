...
class DataMesh:
    ...
    async def ingest(self, source_id: str, query: DataQuery) -> Optional[RawPacket]:
        ...
        try:
            ...
            packet = await asyncio.wait_for(
                source.ingest(query), timeout=self.default_timeout_ms / 1000
            )
            packet.latency_ms = (time.monotonic() - t0) * 1000
            return packet
        except asyncio.TimeoutError:
            logger.warning(
                f"DataMesh: source '{source_id}' timed out ({self.default_timeout_ms}ms)"
            )
            return RawPacket(
                source_id=source_id,
                data=None,
    ...
    async def ingest_parallel(
        ...
        tasks = {
            sid: asyncio.create_task(self.ingest(sid, q)) for sid, q in queries.items()
        }
        results = {}
    ...
    async def ingest_best_effort(
        self, source_ids: List[str], query_template: DataQuery
    ) -> List[RawPacket]:
        queries = {
            sid: DataQuery(
                source_id=sid,
                market=query_template.market,
                ticker=query_template.ticker,
                limit=query_template.limit,
                extra=query_template.extra,
            )
