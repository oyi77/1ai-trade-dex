...
def _reject_unsafe(plugin_class: type) -> None:
    ...
    if manifest.requires_db:
        raise ValueError(f"Node '{manifest.name}' requires database access - not allowed in sandbox")
    if manifest.requires_live_data:
        raise ValueError(f"Node '{manifest.name}' requires live data - not allowed in sandbox")
