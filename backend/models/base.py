"""Database connection, session infrastructure, and schema management.

This is the foundation module for the models package. It provides:
- SQLAlchemy engine, session factory, and declarative Base
- Connection listeners (SQLite WAL, PostgreSQL lock timeouts)
- Schema migration helpers (ensure_schema, seed_default_data)
- Data recovery helpers for corrupted SQLite databases
...
def configure_sqlite_wal(engine_obj):
    ...
    @sqla_event.listens_for(engine_obj, "connect")
...
# Side-effect imports: ensure model modules register their tables with Base
try:
...
# ---------------------------------------------------------------------------
# Data recovery helpers (low-level SQLite, no model imports needed at module level)
# ---------------------------------------------------------------------------
...
def _restore_recovered_data(recovered: dict[str, list[dict]]):
    ...
    Model classes are imported lazily to avoid circular imports at module load time.
    """
    if not recovered:
        return

    # Lazy imports — model modules are fully loaded by the time this is called
...
def _seed_default_bot_state(db, mode: str, app_settings):
    """Helper: create or update BotState for one mode.

    Uses a lazy import for BotState to avoid circular dependencies.
    """
    from backend.models.bot import BotState
...
def seed_default_data():
    ...
    try:
        ...
        for mode in ["paper", "testnet", "live"]:
            _seed_default_bot_state(db, mode, app_settings)

        from backend.strategies.loader import load_all_strategies
        from backend.strategies.registry import STRATEGY_REGISTRY
        from backend.models.config_models import StrategyConfig
...
def ensure_schema():
    ...
    # Ensure copy_trader_entries table exists — needs a lazy import
    try:
    ...
    if "copy_trader_entries" not in copy_entry_tables:
        from backend.models.market_data import CopyTraderEntry
    ...
    if "settlement_events" not in copy_entry_tables:
        from backend.models.event import SettlementEvent
    ...
    # Create composite indexes on genome_registry (idempotent)
    try:
        with engine.connect() as conn:
            with conn.begin():
                conn.execute(
...
def log_audit(action: str, actor: str = "system", details: dict = None):
    """Legacy audit helper — uses a lazy import for AuditLog."""
    from backend.models.audit import AuditLog
...
# Deferred import: register StrategyPerformanceSnapshot without failing on circular deps
try:
