...
class FewShotLearningBenchmark:
    ...
    def run(self, examples: Optional[List[MarketExample]] = None) -> BenchmarkResult:
        ...
        test_examples = examples[
            self.TRAIN_EXAMPLES : self.TRAIN_EXAMPLES + self.TEST_TRADES
        ]
        ...
        test_results = self._evaluate_strategy_on_test_set(
            generated_strategy, test_examples
        )
        ...
        result = BenchmarkResult(
            ...
            metadata={
                "gap": gap,
                "train_success_rate": sum(1 for ex in train_examples if ex.success)
                / len(train_examples),
    ...
    def _generate_synthetic_examples(self) -> List[MarketExample]:
        ...
        train = [
            MarketExample(
                market_type="sports",
                context={"team_strength": 0.8, "home_advantage": True},
                action="bet_home",
                outcome=1.2,
                success=True,
            ),
            MarketExample(
                market_type="sports",
                context={"team_strength": 0.3, "home_advantage": False},
                action="bet_away",
                outcome=0.9,
                success=True,
            ),
            MarketExample(
                market_type="sports",
                context={"team_strength": 0.6, "home_advantage": True},
                action="bet_home",
                outcome=0.9,
                success=False,
            ),
        ...
        for _ in range(self.TEST_TRADES):
            ...
            outcome = (
                ...
                if correct_action != "bet_draw"
                else random.uniform(0.9, 1.1)
            )
            test.append(
                MarketExample(
                    market_type="sports",
    ...
    def _build_few_shot_prompt(self, examples: List[MarketExample]) -> str:
        prompt_parts = [
            "Generate a trading strategy for sports prediction markets using these examples:"
        ]
        for ex in examples:
            prompt_parts.append(
                f"- Context: {ex.context}, Action: {ex.action}, Success: {ex.success}"
            )
        return "\n".join(prompt_parts)
...
"""
    ...
    def _evaluate_strategy_on_test_set(
        self, strategy: GeneratedStrategy, test_set: List[MarketExample]
    ) -> List[bool]:
        results = []
    ...
    def _predict_from_strategy(
        self, strategy: GeneratedStrategy, context: Dict[str, Any]
    ) -> str:
    ...
    def _compute_gap(
        self, train_examples: List[MarketExample], test_results: List[bool]
    ) -> float:
        train_success_rate = sum(1 for ex in train_examples if ex.success) / len(
            train_examples
        )
    ...
    def register():
        ...
        try:
            ...
            BenchmarkRegistry.register(
                FewShotLearningBenchmark.BENCHMARK_ID, FewShotLearningBenchmark
            )
        except ImportError:
