...
class CrossDomainTransferBenchmark:
    ...
    def run(self, fixtures: Optional[List[TradeFixture]] = None) -> BenchmarkResult:
        ...
        few_shot_prompt = self._build_few_shot_prompt(
            fixtures[: self.FEW_SHOT_EXAMPLES]
        )
        adapted_strategy = self._adapt_strategy_to_target(
            source_strategy, few_shot_prompt
        )

        simulated_results = self._run_simulated_trades(
            adapted_strategy, self.SIMULATED_TRADES
        )
        random_baseline = self._run_random_baseline(self.SIMULATED_TRADES)

        transfer_rate = self._compute_transfer_success_rate(
            simulated_results, random_baseline
        )
        ...
        result = BenchmarkResult(
            ...
            metadata={
                ...
                "adapted_win_rate": sum(1 for r in simulated_results if r)
    ...
    def _generate_synthetic_fixtures(self) -> List[TradeFixture]:
        return [
            TradeFixture(
                domain="crypto",
                market_context={"trend": "up", "volatility": 0.3},
                trade_action="buy",
                outcome=1.05,
                success=True,
            ),
            TradeFixture(
                domain="crypto",
                market_context={"trend": "down", "volatility": 0.5},
                trade_action="sell",
                outcome=0.95,
                success=False,
            ),
            TradeFixture(
                domain="weather",
                market_context={"temp_change": 5, "precipitation": 0.2},
                trade_action="long",
                outcome=1.02,
                success=True,
            ),
            TradeFixture(
                domain="weather",
                market_context={"temp_change": -3, "precipitation": 0.8},
                trade_action="short",
                outcome=1.08,
                success=True,
            ),
            TradeFixture(
                domain="weather",
                market_context={"temp_change": 0, "precipitation": 0.5},
                trade_action="hold",
                outcome=1.0,
                success=True,
            ),
    ...
    def _build_few_shot_prompt(self, examples: List[TradeFixture]) -> str:
        prompt_parts = [
            "Adapt this crypto strategy to weather markets using these examples:"
        ]
        for ex in examples:
            prompt_parts.append(
                f"- Context: {ex.market_context}, Action: {ex.trade_action}, Success: {ex.success}"
            )
        return "\n".join(prompt_parts)

    def _adapt_strategy_to_target(
        self, source: StrategySpec, few_shot_prompt: str
    ) -> StrategySpec:
        return StrategySpec(
    ...
    def _run_simulated_trades(
        self, strategy: StrategySpec, n_trades: int
    ) -> List[bool]:
        results = []
    ...
    def _compute_transfer_success_rate(
        self, adapted_results: List[bool], baseline_rate: float
    ) -> float:
...
def register():
    ...
    try:
        ...
        BenchmarkRegistry.register(
            CrossDomainTransferBenchmark.BENCHMARK_ID, CrossDomainTransferBenchmark
        )
