    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      {filtered.length > 0 && (
        <div className="shrink-0 px-3 py-1.5 border-t border-neutral-800 flex items-center gap-4 text-[9px] text-neutral-600">
          ...
          <span>Top score: <span className="text-amber-400 tabular-nums">{filtered.length > 0 ? filtered.reduce((max, t) => Math.max(max, t.score ?? 0), filtered[0]?.score ?? 0).toFixed(2) : '0.00'}</span></span>
        </div>
      )}
    </div>
  )
