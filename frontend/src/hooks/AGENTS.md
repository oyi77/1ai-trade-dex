...
## Purpose
Custom React hooks encapsulating shared business logic: authentication state management, real-time WebSocket connection handling with automatic reconnection, dashboard stats polling, server-side paginated table data fetching with sorting/filtering, and real-time trade event streaming.
...
| `useStats.ts` | Dashboard stats polling hook — fetches bot statistics (total return, Sharpe, win rate, max drawdown, etc.) on 10s interval, uses React Query for caching |
...
| `useSSEEvents.ts` | Server-Sent Events hook — handles real-time data streams via SSE protocol |
...
| `useModeFilter.ts` | Filter mode hook — manages different filtering modes and system states |

## Subdirectories
None — all hooks are at root level of `hooks/`.
...
- useStats polls every 10 seconds with `refetchInterval: 10000`
...
- useTradeEvents tests: mock WebSocket subscription, verify message parsing, verify error recovery
- All tests use `renderHook()` from @testing-library/react, avoid testing localStorage directly
...
### Internal
- `../api.ts` — api client instance, adminApi client, fetch function calls (getAdminApiKey, setAdminApiKey, fetch functions)
- `../types.ts` — type definitions for BotStats, Trade, Signal, etc.

### External
- `react@18` — useState, useEffect, useRef, useCallback, useQuery, etc.
